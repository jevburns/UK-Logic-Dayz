class CampMission extends SurvivorMissions
{
	...
	{				
		...
		//Get random loadout 
		int selectedLoadout = Math.RandomIntInclusive(0,0);	//!change randomization limit after adding new loadouts!	
		...
		{
			weapon = MissionObject.GetInventory().CreateInInventory("KOD_SCARH");
			MissionObject.GetInventory().CreateInInventory("KOD_SCARH");
