class ApartmentMission extends SurvivorMissions
{
	...
	{	
		...
		ItemBase MissionObject = ItemBase.Cast( GetGame().CreateObject( "Msp_MilitaryCrate_Brown", m_MissionPosition, false, false, false ) );
		
		//Get random loadout 			
		int selectedLoadout = Math.RandomIntInclusive(0,4);	//!Change randomization limit after adding new loadouts!
		...
		if (selectedLoadout == 4)
		{
			...
			MissionObject.GetInventory().CreateInInventory("Ammo_762x39");
			weapon = MissionObject.GetInventory().CreateInInventory("A2G36_C");
			MissionObject.GetInventory().CreateInInventory("A2AK74S");
